...
class StarboardEvents:
        ...
        if message.embeds:
            ...
            for em in embeds:
                ...
                if em.description is not None:
                    em.description = "{}\n\n{}".format(message.system_content, em.description)[:4096]
                else:
            ...
            if starboard.colour in ["user", "member", "author"]:
                ...
                em.color = discord.Colour(starboard.colour)
            em.description = message.system_content
            em.set_author(name=author.display_name, url=message.jump_url, icon_url=author.display_avatar)
            if msg_ref := getattr(message, "reference", None):
                ref_msg = getattr(msg_ref, "resolved", None)
                if ref_msg is not None:
                    try:
            ...
            if message.attachments:
                for attachment in message.attachments:
                    ...
                    elif not attachment.filename.lower().endswith(("png", "jpeg", "jpg", "gif", "webp")):
                        new_em.add_field(name="Attachment", value=f"[{attachment.filename}]({attachment.url})")
                    else:
    ...
    async def _update_stars(self, payload: Union[discord.RawReactionActionEvent, FakePayload]) -> None:
        """
        ...
        use_any_emoji = await self.config.guild(guild).use_any_emoji()
        guild_emojis = [e.id for e in guild.emojis]
        emoji = payload.emoji
        for starboard in self.starboards[guild.id].values():
            if starboard.emoji == payload.emoji or (
                use_any_emoji and (isinstance(emoji, str) or emoji.id in guild_emojis or emoji.is_unicode_emoji())
            ):
                ...
                for message_ids, message in entry.messages.items():
                    if message.author == user_id:
                        ...
                        try:
                            ...
                            del self.starboards[guild_id][starboard].starboarded_messages[index_key]
                        except Exception:
                            pass
            ...
            for guild_id, starboards in self.starboards.items():
                ...
                for name, starboard in starboards.items():
                    async with starboard.lock:
                        ...
                        to_rem_index = []
                        try:
                            async for message_ids, message in AsyncIter(starboard.messages.items(), steps=500):
