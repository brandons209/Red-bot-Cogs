...
class Starboard(StarboardEvents, commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.config.register_guild(starboards={}, use_any_emoji=False)
    ...
    @starboard.command(name="anyemoji")
    async def starboard_anyemoji(self, ctx: commands.Context, on_off: bool):
        """
        Allow any emoji to add a message to a starboard
        If you have multiple boards messages may be posted on all of them depending on the configured number of reactions for a message to go onto a board.
        """
        await self.config.guild(ctx.guild).use_any_emoji.set(on_off)
        await ctx.tick()
    ...
    async def format_starboard(self, ctx: commands.Context, starboard: StarboardEntry) -> discord.Embed:
        guild = ctx.guild
        ...
        msg = _(
            ...
            "Any emoji will add entry: {any_emoji}\n"
        ).format(
            name=starboard.name,
            ...
            any_emoji=await self.config.guild(ctx.guild).use_any_emoji(),
        )
    ...
    async def setup_starboard(
        ...
        msg = _("Starboard set to {channel} with emoji {emoji}").format(channel=channel.mention, emoji=emoji)
        await ctx.send(msg)
            ...
            if starboard.whitelist:
                for c in starboard.whitelist:
                    ...
                    if channel is None and role is None:
                        ...
                        channels += 1
        await self._save_starboards(guild)
        msg = _("Removed {channels} channels and roles, and {boards} boards " "that no longer exist").format(
            channels=channels, boards=boards
        )
        await ctx.send(msg)
    ...
    async def star(
        ...
        if not starboard.check_roles(ctx.message.author):
            error_msg = _(
                "One of your roles is blocked on {starboard} " "or you don't have a role that is allowed."
    ...
    async def inherit(
        ...
        if starboard.inherit:
            msg = _("Starboard {name} will now inherit parent channel settings.").format(name=starboard.name)
        else:
            msg = _("Starboard {name} will not check if the parent channel is blocked.").format(name=starboard.name)
        await ctx.send(msg)
    ...
    async def change_channel(
        ...
        await self._save_starboards(guild)
        msg = _("Starboard {name} set to post in {channel}").format(name=starboard.name, channel=channel.mention)
        await ctx.send(msg)
    ...
    async def colour_starboard(
        ...
        await self._save_starboards(guild)
        msg = _("Starboard `{name}` colour set to `{colour}`.").format(name=starboard.name, colour=starboard.colour)
        await ctx.send(msg)
    ...
    async def set_emoji(
        ...
        if type(emoji) == discord.Emoji:
            if emoji not in guild.emojis:
                await ctx.send(_("That emoji is not on this guild!"))
                return
        self.starboards[ctx.guild.id][starboard.name].emoji = discord.PartialEmoji.from_str(str(emoji))
        await self._save_starboards(guild)
    ...
    async def set_threshold(
        ...
        await self._save_starboards(guild)
        msg = _("Threshold of {threshold} reactions set for {name}").format(threshold=threshold, name=starboard.name)
        await ctx.send(msg)
