...
class Follower(commands.Cog):
    ...
    async def unfollow(
        ...
        if isinstance(user, int):
            ...
            user_id = user.id
            followers = await self.config.user(user).followers()
            last_active_time = await self.config.user(user).last_active_time()
        ...
        if isinstance(author, int):
            ...
            author_id = author.id
            following = await self.config.user(author).following()
        ...
        await self.config.user_from_id(user_id).followers.set(followers)
        await self.config.user_from_id(user_id).last_active_time.set(last_active_time)
        await self.config.user_from_id(author_id).following.set(following)
    ...
    async def followers_list(self, ctx: commands.Context):
        """
    ...
    async def followers_list_followers(self, ctx: commands.Context):
        """
    ...
    async def followers_list_following(self, ctx: commands.Context):
        """
    ...
    async def followers_list_blocked(self, ctx: commands.Context):
        """
    ...
    async def followers_opt_out(self, ctx: commands.Context, on_off: bool):
        """
    ...
    async def followers_block(self, ctx: commands.Context, *, user: discord.User):
        """
    ...
    async def followers_unblock(self, ctx: commands.Context, *, user: discord.User):
        """
    ...
    async def followers_unfollow(
        ...
        user: discord.User,
        *,
        channel: Optional[Union[discord.TextChannel, discord.Thread, discord.VoiceChannel]] = None,
    ):
        """
        Unfollow a user.
    ...
    async def followers_follow(
        ...
        **Make sure to turn on allow DMs from me so I can notify you!**
        ...
        if not member:
            await ctx.send(
                error(
                    f"You don't appear to be in the server {channel.guild.name}\n\nIf this is a mistake, contact the bot owner."
                )
            )
            return
