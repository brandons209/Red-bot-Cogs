...
class IntervalsModal(discord.ui.Modal, title="Timing Settings"):
    def __init__(self, panel, poll, rearm_grace, rearm_pct, cooldown, rename):
        ...
        self.margin = discord.ui.TextInput(
            label="Re-arm margin (% of threshold)", default=str(rearm_pct), required=True, max_length=3
        )
    ...
    async def on_submit(self, interaction):
        try:
            ...
            pct = int(self.margin.value.strip())
        ...
        await self.panel.cog._set_interval(self.panel.guild, "rearm_pct", pct)
...
class ServerWatchPanel(discord.ui.View):
    ...
    def _build_intervals(self, s):
        ...
        async def cb(interaction):
            ...
            pct = await gconf.rearm_pct()
            cd = await gconf.cooldown()
            rn = await gconf.rename_interval()
            await interaction.response.send_modal(IntervalsModal(self, poll, rg, pct, cd, rn))
    ...
    async def _add_intervals_field(self, embed):
        ...
        embed.add_field(
            ...
            value=(
                ...
                f"Re-arm margin: **{pct}%** of a threshold (alert re-arms only below this)\n"
                f"Re-arm grace: **{rg}s** sustained below the floor before it re-arms\n"
